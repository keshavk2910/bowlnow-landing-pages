...
export default function bowlingleague8jz0ba({
  ...
  return (
    <div className='min-h-screen bg-white'>
      ...
      {book_your_event_1.enabled && (
        <section className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-15'>
          <BookYourEventHalf1
            content={book_your_event_1}
            themeColor={themeColor}
          />
        </section>
      )}
      ...
      {league_schedule.enabled && (
        <section className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16'>
          <div className='text-center mb-12'>
            <h2 className='text-3xl md:text-5xl font-bold text-black mb-4'>
              {league_schedule.section_title || 'Bowling League Schedule'}
            </h2>
            {league_schedule.section_subtitle && (
          ...
          {league_schedule.schedule_table && Array.isArray(league_schedule.schedule_table) && league_schedule.schedule_table.length > 0 && (
            <div className='bg-white rounded-lg shadow-lg overflow-hidden'>
              <div className='overflow-x-auto'>
                <table className='w-full'>
                  ...
                  <tbody className='bg-white divide-y divide-gray-200'>
                    {league_schedule.schedule_table.map((row, index) => (
              ...
              {league_schedule.schedule_note && (
                <div className='bg-gray-50 px-6 py-4 border-t'>
                  <p className='text-sm text-gray-600'>
                    <span className='font-medium'>Note:</span> {league_schedule.schedule_note}
                  </p>
                </div>
              )}
              
              {league_schedule.contact_info && (
                <div className='bg-blue-50 px-6 py-4 border-t'>
                  <p className='text-sm text-blue-700'>
                    <span className='font-medium'>Sign up or get more info:</span> {league_schedule.contact_info}
                  </p>
                </div>
              )}
            </div>
