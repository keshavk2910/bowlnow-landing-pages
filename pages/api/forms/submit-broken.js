...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (site.ghl_location_id && site.ghl_location_token_encrypted) {
      ...
      syncToGHLAsync(site, page_id, form_data, form_type, customer, attribution_data, session_id, submission.id)
    ...
    res.status(200).json({
      ...
      customer: customer,
      message: 'Form submitted successfully, processing integrations in background'
    });
...
async function syncToGHLAsync(site, page_id, form_data, form_type, customer, attribution_data, session_id, submissionId) {
  try {
    ...
    if (page_id) {
      ...
      // Get pipeline configuration in same query
      var pageSpecificPipeline = pageData?.pipeline_id;
      var pageSpecificMappings = pageData?.stage_mappings || {};
    }
    ...
    let stageMappings = pageSpecificMappings || site.default_stage_mappings || {};
