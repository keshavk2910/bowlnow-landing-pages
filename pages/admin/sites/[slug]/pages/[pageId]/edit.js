...
export default function EditPageMultiStep() {
      ...
      if (field.type === 'slider') {
        ...
        if (field.required || minSlides > 0) {
          if (slides.length < minSlides) {
            errors.push(
              `"${field.label}" requires at least ${minSlides} slides (currently has ${slides.length})`
            );
          }
        }
  ...
  async function handleUpdatePage() {
    ...
    try {
      ...
      console.log('Sending page content update:', {
        content: pageContent,
  ...
  const canProceedToStep2 =
    basicInfo.name && basicInfo.slug && basicInfo.template_id;
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleBasicInfoChange('page_type', e.target.value)
                      }
                      className='mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500'
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleBasicInfoChange('template_id', e.target.value)
                      }
                      className='mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500'
                    >
                ...
                <div className='space-y-4'>
                  <div className='flex items-center'>
                    <input
                      ...
                      onChange={(e) =>
                        handleBasicInfoChange('is_homepage', e.target.checked)
                      }
                  ...
                  <div className='flex items-center'>
                    <input
                      ...
                      onChange={(e) =>
                        handleBasicInfoChange('is_published', e.target.checked)
                      }
                      className='h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded'
                    />
                    <label
                ...
                {template && (
                  <div className='bg-blue-50 rounded-lg p-4'>
                    ...
                    <div className='text-xs text-blue-600'>
                      Fields: {template.config_schema.fields?.length || 0}{' '}
                      customizable fields
                    </div>
                  </div>
                )}
        ...
        {currentStep === 2 && template && (
          <div className='bg-white shadow rounded-lg'>
            <div className='px-4 py-5 sm:p-6'>
              <div className='mb-6'>
                <h3 className='text-lg leading-6 font-medium text-gray-900'>
                  Edit Page Content & Pipeline
                </h3>
                <p className='mt-1 text-sm text-gray-500'>
                  Update content for "{basicInfo.name}" and configure pipeline
                  settings.
                </p>
              </div>
